...
# Usage: sh -s <MITM_PORT> <CONTROL_PORT> <TARGET_CGROUP>
...
if ! ensure_rule -t nat -C OUTPUT -p tcp -j REDIRECT --to-ports "$MITM_PORT"; then
    apply_rule "nat OUTPUT redirect TCP" ip6tables_cmd -t nat -A OUTPUT -p tcp -j REDIRECT --to-ports "$MITM_PORT"
fi
...
if [ -n "$TARGET_CGROUP" ] && [ -n "$CONTROL_PORT" ]; then
    if ! ensure_rule -t filter -C OUTPUT -m cgroup --path "$TARGET_CGROUP" -p tcp --dport "$CONTROL_PORT" -j REJECT; then
        if ip6tables_cmd -t filter -A OUTPUT -m cgroup --path "$TARGET_CGROUP" -p tcp --dport "$CONTROL_PORT" -j REJECT --reject-with tcp-reset 2>&1; then
            echo "leash: blocked target cgroup $TARGET_CGROUP from reaching control plane port $CONTROL_PORT (IPv6)"
        else
...
if [ -n "$CONTROL_PORT" ]; then
    if ! ensure_rule -t filter -C OUTPUT -o lo -d ::1 -p tcp --dport "$CONTROL_PORT" -j REJECT --reject-with tcp-reset; then
        apply_rule "filter OUTPUT block loopback to control port" ip6tables_cmd -t filter -A OUTPUT -o lo -d ::1 -p tcp --dport "$CONTROL_PORT" -j REJECT --reject-with tcp-reset
    fi
fi
...
if [ "$RULE_ERRORS" -gt 0 ]; then
    echo "leash: WARNING: $RULE_ERRORS ip6tables rule(s) failed to apply (network interception may be incomplete)" >&2
fi
exit 0
